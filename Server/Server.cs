        ...
        lock (_dbLock)
        {
            try
            {
                using (var connection = new SqliteConnection(connectionString))
                {
                    ...
                    cmd.CommandText = @"CREATE TABLE IF NOT EXISTS Dados (Id INTEGER PRIMARY KEY AUTOINCREMENT, GatewayId TEXT, SensorId TEXT, Zona TEXT, TipoDado TEXT, Valor TEXT, Timestamp TEXT, IsAlarm INTEGER DEFAULT 0)";
                    cmd.ExecuteNonQuery();
                }
            }
        ...
        lock (_dbLock)
        {
            try
            {
                using (var connection = new SqliteConnection(connectionString))
                {
                    connection.Open();
                    var cmd = connection.CreateCommand();
                    cmd.CommandText = @"INSERT INTO Dados (GatewayId, SensorId, Zona, TipoDado, Valor, Timestamp, IsAlarm) VALUES ($gatewayId, $sensorId, $zona, $tipoDado, $valor, $timestamp, $isAlarm)";
