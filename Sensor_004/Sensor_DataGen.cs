...
namespace sensor
{
    partial class Program
    {
        ...
        static void GerarEEnviarDado(SensorConfig cfg)
        {
            ...
            // QueryString
            string valFormatado = v.ToString(CultureInfo.InvariantCulture);
            string queryString = $"cmd=DATA_SEND&id={_idSensor}&tipo={tipo}&val={valFormatado}&ts={ts}";

            string tag = extra > 0 ? " [ONDA]" : "";
            RegistarLog($"{tipo}: QueryString gerada.{tag}");

            Publicar(queryString, $"{_zona}.{tipo}");
        }
