...
# env vars injected inline so each child window gets them regardless of inheritance
...
$pids += (Start-Process powershell -ArgumentList '-NoExit', '-Command', "$envs; cd '$root\Gateway'; dotnet run -- '$root\configs\gateways\Gateway_001'" -PassThru).Id
Start-Sleep 3
