...
namespace sensor
{
    partial class Program
    {
        ...
        static void GerarEEnviarDado(SensorConfig cfg)
        {
            ...
            // S002 publishes JSON so PreProcessamento exercises format-detection + unit conversion
            double publishVal = tipo == "TEMP" ? Math.Round(v * 9.0 / 5.0 + 32.0, 1) : v;
            string unidade    = tipo switch { "TEMP" => "F", "HUM" => "%", "CO2" => "ppm", _ => "" };
            string pubVal     = publishVal.ToString(CultureInfo.InvariantCulture);
            string payload    = $"{{\"sensor_id\":\"{_idSensor}\",\"tipo_dado\":\"{tipo}\",\"valor\":{pubVal},\"unidade\":\"{unidade}\",\"timestamp\":\"{ts}\"}}";

            RegistarLog($"{tipo}: {pubVal}{unidade} [JSON]{evTag}");
            Publicar(payload, $"{_zona}.{tipo}");
        }
