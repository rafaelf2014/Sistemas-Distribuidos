...
namespace sensor
{
    partial class Program
    {
        ...
        static void GerarEEnviarDado(SensorConfig cfg)
        {
            ...
            string idNumerico = _idSensor.ToUpper().Replace("S", "");
            byte byteId = byte.TryParse(idNumerico, out byte idRes) ? idRes : (byte)99;

            byte byteTipo = tipo.ToUpper() switch {
                "TEMP"  => 0x0A,
                "HUM"   => 0x0B,
                "CO2"   => 0x0C,
                "LUMIN" => 0x0D,
            ...
            short valorCurto = (short)Math.Round(v * 10);

            string hexPayload = $"{byteId:X2}{byteTipo:X2}{valorCurto:X4}";

            string tag = extra > 0 ? " [ONDA]" : "";
            RegistarLog($"{tipo}: HEX gerado -> {hexPayload}{tag}");
            
            Publicar(hexPayload, $"{_zona}.{tipo}");
        }
