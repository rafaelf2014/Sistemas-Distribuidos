...
namespace sensor
{
    partial class Program
    {
        ...
        static void GerarEEnviarDado(SensorConfig cfg)
        {
            ...
            // XML
            XElement xmlPayload = new XElement("Mensagem",
                new XAttribute("tipo", "DATA_SEND"),
                new XElement("SensorId", _idSensor),
                new XElement("Variavel", tipo),
                new XElement("Valor", v.ToString(CultureInfo.InvariantCulture)),
                new XElement("DataHora", ts)
            );

            string xmlStr = xmlPayload.ToString(SaveOptions.DisableFormatting);    
            string tag = extra > 0 ? " [ONDA]" : "";

            RegistarLog($"{tipo}: XML gerado.{tag}");
            
            Publicar(xmlStr, $"{_zona}.{tipo}");
        }
